...
התוצאה: /var/www/html/app/pluck-4.7.13/files

המשמעות: אני נמצא בתיקיית העלאת הקבצים של האתר. זהו ה"נמל" שדרכו נכנסתי למערכת.

💡 למה אני עושה את זה ובשביל מה?
אימות הרשאות: למרות שאני נמצא בתיקיית האתר, אני רוצה לבדוק אם כמשתמש www-data יש לי הרשאות קריאה לתיקיות של משתמשים אחרים.

מציאת הדגל הראשון (The User Flag): המטרה שלי היא לקרוא את קובץ ה-user.txt של Lucien. בדרך כלל הדגל נמצא בתיקיית הבית (home) של המשתמש.
