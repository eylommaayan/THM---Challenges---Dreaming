...
המטרה: ניצול פגיעות במערכת הניהול (CMS) כדי להשיג דריסת רגל (Foothold) בשרת והרצת פקודות מרחוק.

2.1  הכנת "ארגז הכלים" ופתרון תקלות

בשלב זה ניסיתי להריץ את האקספלויט נגד היעד, אך נתקלתי בשגיאה במכונה התוקפת שלי.

הפעולה: ניסיון הרצת הסקריפט exploit.py.

הבעיה: הטרמינל החזיר שגיאת No such file or directory. הבנתי שהסקריפט לא קיים מקומית בתיקיית העבודה שלי.

הפתרון: הורדתי את האקספלויט המתאים (CVE-2020-29607) ממאגר Exploit-DB הרשמי.

הפקודה שביצעתי:

Bash
wget https://www.exploit-db.com/download/49909 -O exploit.py
2.2 הרצת ה-Exploit והשגת Webshell.
